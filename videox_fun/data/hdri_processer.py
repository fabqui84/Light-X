...
class HDRI_Preprocessor:
    ...
    def load_hdri(self, hdri_file_path):
        ...
        self.envmap_h = self.hdri.shape[0]
        self.envmap_w = self.hdri.shape[1]
    
    # def load_hdri(self,hdri_file_path):
    #     """Load .hdr HDRI image using OpenCV and convert to RGB torch tensor."""
    #     hdr = cv2.imread(hdri_file_path, flags=cv2.IMREAD_UNCHANGED)  # [H, W, 3] in BGR
    #     if hdr is None:
    #         raise FileNotFoundError(f"Failed to read HDR image: {hdri_file_path}")
    #     hdr = cv2.cvtColor(hdr, cv2.COLOR_BGR2RGB)                     # Convert to RGB
    #     self.hdri = np.clip(hdr, 0.0, None).astype(np.float32)               # Ensure non-neg
    #     self.hdri = torch.from_numpy(self.hdri)
    ...
    def get_rotate_hdri_cond(self, hdri_rot_roll):
        ...
        hdri_cond = envir_map_ldr.float() # [3, H, W]
    ...
    def preprcess_envir_map(self, envir_map, hdri_rot, flip=False, debug=False):
        ...
        envir_map_ldr = F.interpolate(
            envir_map_ldr.permute(2, 0, 1).unsqueeze(0),
            size=(384, 672),
            mode='bilinear',
            align_corners=True
        ).squeeze()

        envir_map_hdr = F.interpolate(
            envir_map_hdr.permute(2, 0, 1).unsqueeze(0),
            size=(384, 672),
            mode='bilinear',
            align_corners=True
        ).squeeze()

        envir_map_hdr_raw = F.interpolate(
            envir_map_hdr_raw.permute(2, 0, 1).unsqueeze(0),
            size=(384, 672),
            mode='bilinear',
            align_corners=True
        ).squeeze()

        view_dirs_world = F.interpolate(
            view_dirs_world.permute(2, 0, 1).unsqueeze(0),
            size=(384, 672),
            mode='bilinear',
            align_corners=True
        ).squeeze()
