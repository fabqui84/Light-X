...
class LightCrossAttentionBlock(nn.Module):
    def __init__(
        self,
        dim: int,
        dim_head: int,
        ...
        self.cross_light_cross_attention = nn.ModuleList([
        ...
        self.cross_query_tokens = nn.Parameter(torch.empty(1, num_queries, dim))

    def init_weights(self):
        if not torch.is_meta(self.cross_query_tokens):
            nn.init.normal_(self.cross_query_tokens, std=0.02)

    def forward(self, light_cross_hidden_states):
        """
        Args:
            light_cross_hidden_states: (B, N, D)
        Returns:
            (B, N, D): attended output
        """
        cross_query_tokens = self.cross_query_tokens.expand(
            light_cross_hidden_states.shape[0], -1, -1
        )  # (B, Q, D)

        for layer in self.cross_light_cross_attention:
            light_cross_hidden_states = cross_query_tokens + layer(
                light_cross_hidden_states, cross_query_tokens
            )
        return light_cross_hidden_states
...
class CogVideoXPatchEmbed(nn.Module):
    def __init__(
        ...
        bias: bool = True,
        cross_attn_dim_head=None, 
        cross_attn_num_heads=None, 
        kv_dim=None
    ) -> None:
        super().__init__()
        ...
        self.light_cross_attention = nn.ModuleList([
            PerceiverCrossAttention(
                dim=embed_dim,
                dim_head=cross_attn_dim_head,
                heads=cross_attn_num_heads,
                kv_dim=3072,
            )
            for _ in range(num_cross_layers)
        ])

    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor, light_embeds: torch.Tensor):
        r"""
        Args:
        ...
        query_tokens = self.query_tokens.expand(light_embeds.shape[0], -1, -1)  # (B, Q, D)
        for layer in self.light_cross_attention:
            query_tokens = query_tokens + layer(light_embeds, query_tokens)  # residual
        embeds = torch.cat(
            [text_embeds, query_tokens, image_embeds], dim=1
        ).contiguous()  # [batch, seq_length + num_frames x height x width, channels]
        return embeds, query_tokens.shape[1]
...
class CrossTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.patch_embed = CogVideoXPatchEmbed(
            ...
            cross_attn_dim_head=cross_attn_dim_head, cross_attn_num_heads=cross_attn_num_heads, kv_dim=None
        )
        ...
        if is_train_cross:
            ...
            self.light_patch_embed = RefPatchEmbed(
                patch_size, cross_attn_in_channels, inner_dim, bias=True,
            )
            self.light_text_patch_embed = RefPatchEmbed(
                patch_size, cross_attn_in_channels, inner_dim, bias=True,
            )
    ...
    def _init_cross_inputs(self):
        ...
        self.light_perceiver_cross_attention = nn.ModuleList(
            [
                PerceiverCrossAttention(
                    dim=self.inner_dim,
                    dim_head=self.cross_attn_dim_head,
        ...
        self.light_cross_block = LightCrossAttentionBlock(
            dim=self.inner_dim,
            dim_head=self.cross_attn_dim_head,
            heads=self.cross_attn_num_heads,
            kv_dim=self.cross_attn_kv_dim,
    ...
    def forward(
        ...
        light_latents: Optional[torch.Tensor] = None,
        image_rotary_emb: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        ...
        light_text_hidden_states = self.light_text_patch_embed(light_latents.clone())
        hidden_states, light_length = self.patch_embed(encoder_hidden_states, hidden_states, light_text_hidden_states)
        if self.is_train_cross:
            cross_hidden_states = self.ref_patch_embed(cross_latents)
            light_cross_hidden_states = self.light_patch_embed(light_latents)
            light_cross_hidden_states = self.light_cross_block(light_cross_hidden_states)
           
        # 3. Position embedding
        text_seq_length = encoder_hidden_states.shape[1]
        text_seq_length = text_seq_length + light_length
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            if self.is_train_cross:
                if i % self.cross_attn_interval == 0:
                    ...
                    hidden_states = hidden_states + self.light_perceiver_cross_attention[
                        ca_idx
                    ](
                        light_cross_hidden_states, hidden_states
                    )  # torch.Size([2, 32, 2048])  torch.Size([2, 17550, 3072])
                    ca_idx += 1
